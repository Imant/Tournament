...
namespace OrchardHUN.TrainingDemo
{
    ...
    {
        // Migrations have Create() and UpdateFromX methods. When the module is first enabled the Create() is called so it can set up
        // DB tables.
        public int Create()
        {
            ...
            // Here we return the number of the migration. If there were no update methods we'd return one. But we have one, see it for
            // more details.
            return 2;
        }

        /*
         * This is an update method. It is used to modify the existing schema. Update methods will be run when the module was already enabled 
         * before and the create method was run.
         * The X in UpdateFromX is the number of the update (the method's name is conventional). It means: "run this update if the module's
         * current migration version is X". This method will run if it's 1.
         */
        public int UpdateFrom1()
        {
             ...
             * Notice that Create() returns 2 as well: this is a good practice. Create() initially had a schema defined where Name was
             * not present. We added it in an update to our module but we not only included it in UpdateFrom1() but also added it to
             * Create()'s table creation logic. What does this mean? Systems already having our module installed will run this update
             * method and modify the table. However where the already updated module is installed only Create() will run.
             */
            return 2;
