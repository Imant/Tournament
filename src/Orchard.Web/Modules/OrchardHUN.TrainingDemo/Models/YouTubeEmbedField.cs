...
namespace OrchardHUN.TrainingDemo.Models
{
    ...
    public class YouTubeEmbedField : ContentField // Note that the class inherits from ContentField
    {
         ...
         * with LINQ and indexed in the DB); but if there can be an unknown number of pieces attached to a content type then fields are the 
         * way to go. You have to keep in mind though that due to the serialization process, fields can potentially perform worse if there are 
         * many items to be fetched. On the other hand parts, since they're stored in separate tables and thus are joined in or even sometimes 
         * loaded lazily one by one may also have negative impact on performance (since the content record, where fields are stored, is always 
         * loaded for a content item there's no subsequent DB call for fetching fields).
         * 
