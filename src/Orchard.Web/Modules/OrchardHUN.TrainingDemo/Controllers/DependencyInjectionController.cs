...
namespace OrchardHUN.TrainingDemo.Controllers
{
    public class DependencyInjectionController : Controller
    {
        ...
        /*
         * The WorkContext is a big object containing all kinds of contextual information including the HttpContext or data of the 
         * current user (if any). Since it's best to access the WorkContext as late as possible we use IWorkContextAccessor to fetch
        ...
        /*
         * IHttpContextAccessor follows the similar pattern as IWorkContextAccessor. Since the HttpContext is included in the WorkContext
         * having an IHttpContextAccessor too is superfluous; we have it here just for the sake of demonstration.
         * 
         * Use IHttpContextAccessor if you want to only access the HttpContext but nothing else from WorkContext so when writing unit tests
         * it's easier to mock it.
         * 
         * We could use the standard ASP.NET statics to access the HttpContext; using this wrapper dependency instead makes unit testing
         * possible.
         * 
